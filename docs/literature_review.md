...
## 2. Deep Dive: Key Reference — Muller et al. (2018)
...
### Key Insights & Direct Mapping to `ghca`

* **Mesoscopic vs. Macroscopic Waves & Axonal Conduction Delays:**  
  Muller et al. establish that mesoscopic travelling waves propagate across unmyelinated horizontal layer 2/3 fibers at $0.1\text{--}0.8\text{ m/s}$, introducing distance-dependent conduction delays that break zero-lag synchrony. In `ghca`, [`ghca_net.py`](../ghca_net.py) and [`ghca_plasticity.py`](../ghca_plasticity.py) directly formalize per-node refractory delays $\tau_i = a + p_i$ and conduction weights $w_{ij}$ to mediate wave propagation across graph topologies.
* **Sparse Waves in Asynchronous Irregular (IA) States:**  
  The review contrasts dense, highly entrained waves (in vitro/epileptiform) with sparse, weakly entraining waves characteristic of the awake asynchronous irregular (IA) state. In `ghca`, homeostatic threshold adaptation $\theta_i \leftarrow \theta_i + \eta_\theta (\bar{\rho}_i - \rho^*)$ and spontaneous firing $p_s$ maintain the substrate in the sparse, self-sustaining spiral band (E0), preventing runaway seizure activity or fixation.
* **Collision-Based Computing & Wave Annihilation:**  
  Muller et al. (§Emerging computational principles) highlight collision-based computation where wave collisions implement logic gates (e.g., XOR via wave annihilation). In `ghca` E4 ([`experiments/e4_attention.py`](../experiments/e4_attention.py)), selective attention is achieved via biased winner-take-all wave annihilation with **zero inhibitory nodes**—the substrate's built-in refractory annihilation supplies the competitive selection.
* **Spatiotemporal Context & Memory Trace:**  
  The review presents the physical analogy of a bouncing droplet on a vibrating oil bath (Perrard et al., 2016), where the global wave field encodes time elapsed since stimulus onset alongside spatial location. In `ghca` E2/E8 ([`experiments/e2_delayed_response.py`](../experiments/e2_delayed_response.py), [`experiments/e8_hardening_results.md`](e8_hardening_results.md)), working memory and time-forward predictions function as persistent $do(\tau)$ reentrant wave loops and history windows.

---

## 3. Candidate Citations by Category

### A. Neuromorphic & Alternative / Unconventional Substrate Computing (NEW)
...
2. **Wright, L. G., Onodera, T., Ng, M. M., et al. (2022).**  
   ...
   * **Relevance & Reasoning:** Demonstrates learning directly on heterogeneous physical substrates (optical, mechanical, electronic). Grounds `ghca`'s framing of excitable cellular automata as a physical/neuromorphic substrate where parameters $\theta$ are adapted directly rather than abstract mathematical weights.
...
3. **Adamatzky, A. (2021).**  
   ...
   * **Relevance & Reasoning:** Comprehensive review of computation using excitable cellular automata, reaction-diffusion substrates, and wave collision dynamics. Provides direct precedence for using Greenberg–Hastings dynamics as a computational medium.
...
5. **Tanaka, G., Yamane, T., Héroux, J. B., et al. (2019).**  
   ...
   * **Relevance & Reasoning:** Grounds `ghca`'s readout paradigm (E6/E9): an un-engineered physical/excitable substrate generates a rich high-dimensional phase state, and linear GVF probes or competitive Hebbian readouts extract cognitive functions without altering the underlying core substrate.

---

### B. Recent Cortical Waves & Spatiotemporal Dynamics (2020–2026)

6. **Muller, L., Chavane, F., Reynolds, J., & Sejnowski, T. J. (2018).**  
   ...
   * **Relevance & Reasoning:** (See Section 2 above). Establishes mesoscopic wave propagation, axonal delays, sparse wave entrainment, and wave-annihilation computation.

7. **Davis, Z. W., Muller, L., Martinez-Trujillo, J., Sejnowski, T. J., & Reynolds, J. H. (2020).**  
   ...
   * **Relevance & Reasoning:** Empirical landmark in awake primates proving that spontaneous travelling waves dynamically gate perceptual sensitivity and neural excitability, directly supporting `ghca`'s wave-annihilation attention mechanism (E4).

8. **Liang, Y., Liang, J., Song, C., Liu, M., Knöpfel, T., Gong, P., & Zhou, C. (2023).**  
   ...
   * **Relevance & Reasoning:** Modern high-resolution optical voltage imaging showing complex topological wave patterns (spirals, sources, sinks) in awake mouse cortex, anchoring the 6 falsifiable spiral wave predictions in Artifact 2b.

9. **Muller, L., Churchland, M. M., & Sejnowski, T. J. (2024).**  
...
10. **Murray, J. D., Bernacchia, A., Wang, X. J., et al. (2014).**  
...
11. **Raut, R. V., Snyder, A. Z., Mitra, A., et al. (2021).**  
    ...
    * **Relevance & Reasoning:** Recent empirical demonstration of infra-slow global waves coordinating local intrinsic timescales and state transitions across cortical hierarchies, supporting `ghca`'s input-timing driven emergent $\tau$-hierarchy (3e.2 / Track 4a).

---

### D. Recent Causal Formulation, Spike–Wave Duality & Causal Emergence (2021–2026)

12. **Jalaldoust, K., & Zabeh, E. (2025).**  
...
13. **Lansdell, B. J., Prakash, P., & Kording, K. P. (2023).**  
...
| :--- | :--- | :--- | :--- |
| **Cortical Waves Review** | Muller et al. (2018) | [`README.md`](../README.md), [`ghca_net.py`](../ghca_net.py) | Cortical Waves & Overview |
...
## 5. Discussion & User Suggestions

> **Note on Full-Text Access:** If any recent paper or paywalled article is required for deeper extraction, please drop the PDF in `docs/lit-rev/` or mention it here.

*Use this space for notes, suggestions, and additions:*

- [ ] **Suggestions on Neuromorphic / Unconventional Computing papers:**
- [ ] **Suggestions on Recent Wave / Intrinsic Timescale papers:**

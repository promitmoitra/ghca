...
| `experiments/clock_shift_healing.py` | Lemma R (proven): step == clock-shift iff dwell-free; live attractors with `S+1 >= 4` are dwell-free, so clock-shift = time-step on them; healing time <= 2S at every `tau_a >= tau_p` cell, equality on the diagonal |
